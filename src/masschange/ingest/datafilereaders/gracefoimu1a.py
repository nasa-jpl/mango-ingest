...
class GraceFOImu1ADataFileReader(AsciiDataFileReader):
    ...
    def get_input_file_default_regex(cls) -> str:
        return '^IMU1A_\d{4}-\d{2}-\d{2}_(?P<stream_id>[CD])_(?P<dataset_version>\d{2})\.txt$'
    ...
    def get_input_column_defs(cls) -> Collection[AsciiDataFileReaderColumn]:
        return [
            ...
            AsciiDataFileReaderColumn(index=4, name='gyro_id', np_type=np.ubyte, unit='implement_me'),  # valid_range: 1, 4
            AsciiDataFileReaderColumn(index=5, name='FiltAng', np_type=np.double, unit='implement_me'),
