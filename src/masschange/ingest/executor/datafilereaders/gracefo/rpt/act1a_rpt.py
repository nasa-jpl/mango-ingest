...
class GraceFOAct1ARptDataFileReader(ReportFileReader):
    @classmethod
    ...
    def get_input_file_default_regex(cls) -> str:
        return '^ACT1A_\d{4}-\d{2}-\d{2}_(?P<instrument_id>[CD])_(?P<dataset_version>\d{2})\.rpt$'
    ...
    def get_rpt_custom_input_column_defs(cls) -> Collection[AsciiDataFileReaderColumn]:
        ...
        return [
            ...
            AsciiDataFileReaderColumn(index=26, name='noutliers', np_type=int, unit=None),
            AsciiDataFileReaderColumn(index=27, name='outlier_max_span', np_type=np.double, unit=None)
        ]
