...
class GraceFOIhk1ADataFileReader(AsciiDataFileReader):
    ...
    def get_input_file_default_regex(cls) -> str:
        return '^IHK1A_\d{4}-\d{2}-\d{2}_(?P<instrument_id>[CD])_(?P<dataset_version>\d{2})\.txt$'
    ...
    def get_input_column_defs(cls) -> Collection[AsciiDataFileReaderColumn]:
        return [
            ...
            AsciiDataFileReaderColumn(index=6, name='sensorvalue', np_type=np.double, unit='implement_me', aggregations=['min', 'max']),
