...
class GraceFOHrt1BDataFileReader(AsciiDataFileReader):
    @classmethod
    ...
    def get_input_file_default_regex(cls) -> str:
        return '^HRT1B_\d{4}-\d{2}-\d{2}_(?P<instrument_id>[CD])_(?P<dataset_version>\d{2})\.txt$'
    ...
    def get_input_column_defs(cls) -> Collection[AsciiDataFileReaderColumn]:
        return [
            ...
            AsciiDataFileReaderColumn(index=33, name='TEMP_USO_pos_y_red', np_type=np.double, unit='degrees C',
                                      aggregations=['min', 'max']),
            ArrayLikeAsciiDataFileReaderColumn(index=34, name='qualflg', np_type='U8', array_size=8)
        ]
