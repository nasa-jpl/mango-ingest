...
class GraceFOGlv1BDataFileReader(AsciiDataFileReader):
    # TODO: from Chris e-mail:
    # File format is the same as GNV1B.  The files span 30 hours centered around one day, so there will be overlap
    # in time between consecutive files – we may need to discuss the best way to handle this.
    ...
    def get_input_file_default_regex(cls) -> str:
        return '^GLV1B_\d{4}-\d{2}-\d{2}_(?P<instrument_id>[CD])_(?P<dataset_version>\d{2})\.txt$'
