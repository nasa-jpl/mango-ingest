...
class GraceFOKbr1AKBandDataFileReader(AsciiDataFileReader):
    """
    ...
    def get_input_file_default_regex(cls) -> str:
        return '^KBR1A_Kband_\d{4}-\d{2}-\d{2}_(?P<instrument_id>[CD])_(?P<dataset_version>\d{2})\.txt$'
    ...
    def get_input_column_defs(cls) -> Collection[AsciiDataFileReaderColumn]:
        return [
            ...
            AsciiDataFileReaderColumn(index=1, name='k_band_freq', np_type=np.double, unit='Hz',
