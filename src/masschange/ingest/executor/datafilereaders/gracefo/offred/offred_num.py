...
class GraceFOOffredNumDataFileReader(OffredFileReader):
    ...
    def get_input_file_default_regex(cls) -> str:
        return '^(?P<instrument_id>GF[12])_CX_[A-Z0-9]+_[A-Z]{3}_(?P<dataset_version>\d{1})_\d+_\d{4}_\d{11}_\d{11}.out'
    ...
    def get_field_value_type(cls) -> np.dtype:
        """
        Return nc_dtype for field_value colunm
        """
        return np.double
