...
def test_gracefo_data_stats(ds: TimeSeriesDataset):
    ...
    test_fields = [f for f in ds.product.get_available_fields() if f.is_valid_statistical_target]
...
def test_qparam_filtering():
    product = GraceFOAcc1ADataProduct()
    ds = TimeSeriesDataset(product, DatasetVersion('04'), 'C')

    def assert_filtered_count_equals(expected_count: int, filter_key: str, filter_values: list[str] = None):
        base_path = f'/missions/{ds.product.mission.id}/products/{ds.product.id_suffix}/versions/{ds.version}/instruments/{ds.instrument_id}/data'

        path = base_path if not filter_values else f'{base_path}?{"&".join([f"filter={filter_key}={v}" for v in filter_values])}'

        response = client.get(path)
        content = response.json()

        assert response.status_code == 200
        assert expected_count == content['data_count']

    assert_filtered_count_equals(600, 'rcvtime_intg', None)
    assert_filtered_count_equals(10, 'rcvtime_intg', ['694310406'])
    assert_filtered_count_equals(20, 'rcvtime_intg', ['694310406', '694310407'])
